...
/**
 * Interactive partner-economics calculator for /jacaruso.
 * Architecture cloned directly from
 * app/grant-hospitality/components/GrantHospitalityCalculator.tsx (itself
 * cloned from app/bridgetown/components/BridgetownCalculator.tsx -- same
 * slider + numeric-input + stepper-button + package-tab pattern), with:
 *   - range 1-50 per the brief (not Jacaruso's actual 3,500+ hotel base)
 *   - default participating-property count of 3
 *   - every output relabeled to Jacaruso's partner-facing terms
 *     (hotel pays / Archer production rate / Jacaruso retains / Jacaruso
 *     gross partner margin)
 *   - no "Bridgetown" or "GRANT" string anywhere in a label, aria-label, or
 *     disclaimer
 *
 * Every figure below is computed live from jacaruso-config.ts, using the
 * exact same formulas as bridgetown-economics.ts, never hardcoded, so the
 * calculator and the static package cards can never drift apart.
 */
...
export function JacarusoCalculator({
  maxProperties,
  defaultProperties,
  defaultTierKey,
}: {
  ...
  return (
    <div className="jac-calc">
      <p className="jac-calc-scenario-label">{CALC_SCENARIO_LABEL}</p>

      <div className="jac-calc-count-row">
        <p className="jac-calc-count">
          <span className="jac-calc-count-num">{propertyCount}</span> participating{" "}
          {pluralizeProperty(propertyCount)}
        </p>
      </div>

      <div className="jac-calc-slider-row mt-4">
        <button
          type="button"
          className="jac-calc-stepper-btn"
        ...
        <div className="jac-calc-slider-wrap">
          <input
            type="range"
            ...
            className="jac-calc-slider"
          />
          <div className="jac-calc-slider-labels" aria-hidden="true">
            {RANGE_LABELS.map((v, i) => (
              <span
                key={v}
                className="jac-calc-slider-label"
                style={{
        ...
        <input
          ...
          className="jac-calc-num-input"
        />
      </div>

      <div className="jac-calc-tabs mt-7" role="tablist" aria-label="Creative activation package">
        {ACTIVATION_TIERS.map((t) => (
          <button
            ...
            className={`jac-calc-tab${tierKey === t.key ? " is-active" : ""}`}
            onClick={() => setTierKey(t.key)}
          >
            {t.name}
            {t.badge ? <span className="jac-calc-tab-badge"> &middot; {t.badge}</span> : null}
          </button>
        ))}
      </div>

      <div className="jac-calc-results mt-6">
        <div className="jac-calc-result-card">
          <p className="jac-calc-result-title">
            {tier.name}
            {tier.badge ? <span className="jac-calc-tab-badge"> &middot; {tier.badge}</span> : null}
          </p>
          <div className="jac-calc-result-rows mt-4">
            <div className="jac-calc-result-row">
            ...
            <div className="jac-calc-result-row jac-calc-result-row--highlight">
              <span>Jacaruso gross partner margin (monthly)</span>
      ...
      <p className="jac-calc-disclaimer mt-6">{CALC_DISCLAIMER_1}</p>
      <p className="jac-calc-disclaimer">{CALC_DISCLAIMER_2}</p>
      <p className="jac-calc-disclaimer">{CALC_DISCLAIMER_3}</p>
    </div>
  );
}
