              ...
              {filtered.map((account) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-4 text-xs text-[#8d8271]">
                      Stage: <span className="font-medium text-[#5f513b]">{label(account.stage)}</span>
                    </div>

                    {/*
                      The two primary actions on every card. Deliberately
                      full-width, side by side, and always visible — not hidden
                      in a menu, an accordion, or revealed on hover. Launching
                      these two things is what this card is for.
                    */}
                    <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                      <Link
                        href={`/client-accounts/${account.id}/preview`}
                        className="inline-flex items-center justify-center gap-2 rounded-2xl border border-[#b89446]/40 bg-[#e8d3a2] px-4 py-3 text-[11px] font-bold uppercase tracking-[0.13em] text-[#3a2f18] transition hover:bg-[#e0c68d]"
                      >
                        <Eye className="h-4 w-4" /> View client dashboard
                      </Link>
                      <Link
                        href={`/client-accounts/${account.id}`}
                        className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#2c251c] px-4 py-3 text-[11px] font-bold uppercase tracking-[0.13em] text-[#f8f3e8] transition hover:bg-[#403426]"
                      >
                        Manage client <ArrowUpRight className="h-4 w-4" />
                      </Link>
                    </div>
                  </article>
                );
